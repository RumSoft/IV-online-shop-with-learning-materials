...
export default class FacebookTokenStrategy extends OAuth2Strategy {
  constructor(_options, _verify) {
    ...
    options.tokenURL =
      options.tokenURL ||
      `https://graph.facebook.com/${_fbGraphVersion}/oauth/access_token`;
    ...
    this._profileURL =
      options.profileURL || `https://graph.facebook.com/${_fbGraphVersion}/me`;
    ...
    this._enableProof =
      typeof options.enableProof === 'boolean' ? options.enableProof : true;
  ...
  userProfile(accessToken, done) {
    ...
    if (this._enableProof) {
      const proof = crypto
        ...
        .digest('hex');
      profileURL.search = `${
        profileURL.search ? profileURL.search + '&' : ''
      }appsecret_proof=${encodeURIComponent(proof)}`;
    }

    // Parse profile fields
    if (this._profileFields) {
      const fields = FacebookTokenStrategy.convertProfileFields(
        this._profileFields
      );
      profileURL.search = `${
        profileURL.search ? profileURL.search + '&' : ''
    ...
    this._oauth2.get(profileURL, accessToken, (error, body, res) => {
      ...
      try {
        ...
        let imageUrl = uri.parse(
          `https://graph.facebook.com/${this._fbGraphVersion}/${
            json.id
          }/picture`
        );
        if (this._profileImage.width)
          imageUrl.search = `width=${this._profileImage.width}`;
        if (this._profileImage.height)
          imageUrl.search = `${
            imageUrl.search ? imageUrl.search + '&' : ''
          }height=${this._profileImage.height}`;
  ...
  lookup(req, field) {
    return (
      ...
      (req.headers &&
        (req.headers[field] || req.headers[field.toLowerCase()])) ||
  ...
  static convertProfileFields(_profileFields) {
    ...
    return profileFields
      .reduce((acc, field) => acc.concat(map[field] || field), [])
      .join(',');
  }
}
